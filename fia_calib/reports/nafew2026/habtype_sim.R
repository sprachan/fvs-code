...
dt <- paste0('lub_habtype', format(Sys.Date(), '%b%d%Y'), '_', 
...
for(i in 1:nrow(lub2018_stands)){
  if(is.na(lub2018_stands$FVS_HAB[i])){
    readcord <- unname(habtype_readcord[['260']])
  }else{
    readcord <- unname(habtype_readcord[[as.character(lub2018_stands$FVS_HAB[i])]])
  }
  if(is.null(readcord)) stop(i)
...
habtype_carb <- bind_rows(carb_out)
rm(carb_out)
habtype_trees <- bind_rows(sim_trees)
rm(sim_trees)

save(habtype_carb, habtype_trees, file = here('reports', 'nafew2026', 'habtype_sim.rda'))